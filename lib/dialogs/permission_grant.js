...
async function expectPermissionResponse(dlg) {
    return dlg.pushStackFrame(new CustomStackFrame(dlg, ValueCategory.PermissionResponse,
        dlg._("I'm waiting for you to grant permission to this request."),
        Intent.Predicate, Intent.PermissionRule, Intent.More, Intent.Yes, Intent.No, Intent.Maybe, Intent.Back));
}
...
async function addFilter(dlg, rule) {
    ...
    for (;;) {
        ...
        */
        ...
        let filterIntent = await expectPermissionResponse(dlg);
        if (filterIntent instanceof Intent.No) {
            await dlg.reset();
            throw new CancellationError(dlg._("User cancelled"));
        }
        if (filterIntent instanceof Intent.Back)
            return null;
        if (filterIntent instanceof Intent.PermissionRule)
            return filterIntent.rule;
        if (!(filterIntent instanceof Intent.Predicate))
            continue;
    ...
    try {
        if (permissionRule) {
            ...
            for (;;) {
                ...
                let answer = await expectPermissionResponse(dlg);
                if (answer instanceof Intent.No) {
                    await dlg.reset();
                    throw new CancellationError(dlg._("User cancelled"));
                }
                if (answer instanceof Intent.Yes)
                    return program;
                if (answer instanceof Intent.PermissionRule) {
                ...
                if (answer instanceof Intent.Maybe) {
