...
function getIdentityName(dlg, identity) {
    ...
    if (split[0] === 'omlet')
        return dlg.interpolate(dlg._("Omlet User @${username}"), { username: split[1] });
...
async function prepareProgram(dlg, program, source) {
    ...
    for (let rule of program.rules) {
        ...
        for (let action of rule.actions) {
            if (action.isNotify && action.name === 'notify')
                hasResult = true;
        }
    }
...
async function displayResult(dlg, app, { hasResult, echo }) {
    ...
    if (!anyResult) {
        ...
        await dlg.maybeReplyChart({ type: 'chart', data: raw }, lastIcon);
    }
}

function confirm(dlg, command, source) {
    return dlg.ask(ValueCategory.YesNo, dlg._("Okay, so you want me to ${command}${? (as asked by ${source})}. Is that right?"), { command, source });
}
...
module.exports = async function ruleDialog(dlg, intent, confident, uniqueId, sourceIdentity) {
    ...
    if (echo) {
        if (source) {
            await dlg.replyInterp(dlg._("I'm going to ${command} (as asked by ${source})."), {
                command: description, source
            });
        } else {
            await dlg.replyInterp(dlg._("Okay, I'm going to ${command}."), {
                command: description
            });
        }
    }
