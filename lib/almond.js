...
module.exports = class Almond extends events.EventEmitter {
    constructor(engine, conversationId, user, delegate, options) {
        ...
        this.parser = new ParserClient(this._options.sempreUrl,
            engine.platform.locale, engine.platform.getSharedPreferences(), engine.platform);
    ...
    handleParsedCommand(root, platformData = {}) {
        ...
        return this._errorWrap(async () => {
            const intent = await Intent.parse(root, this.schemas, this._getContext(null, platformData));
    ...
    handleThingTalk(thingtalk, platformData = {}) {
        ...
        return this._errorWrap(async () => {
            const intent = await Intent.parseThingTalk(thingtalk, this.schemas, this._getContext(null, platformData));
            intent.confident = true;
            return this._doHandleCommand(intent, null, []);
        }, platformData);
    }
    ...
    setContext(context) {
        if (context === null) {
            ...
            const entities = {};
            const code = ThingTalk.NNSyntax.toNN(context, '', entities, {
    ...
    _continueHandleCommand(command, analyzed, platformData) {
        ...
        return Promise.all(analyzed.candidates.map(async (candidate, beamposition) => {
            let parsed;
            try {
                parsed = await Intent.parse({ code: candidate.code, entities: analyzed.entities }, this.schemas, this._getContext(command, platformData));
            } catch(e) {
            ...
            if (candidates.length > 0) {
                ...
                while (i < candidates.length-1 && choice.target instanceof Intent.Unsupported && choice.score === 'Infinity') {
                    i++;
                    choice = candidates[i];
                }

                this.stats.hit('sabrina-command-good');
                choice.target.confident = choice.score === 'Infinity' || this._context.code !== 'null';
                ...
                return this._dispatcher.handle(new Intent.Failed(command, platformData));
            }
        });
    }
    ...
    handleCommand(command, platformData = {}, postprocess) {
        ...
        return this._errorWrap(async () => {
            if (this._raw && command !== null) {
                const value = ThingTalk.Ast.Value.String(command);
                const thingtalk = new ThingTalk.Ast.Input.Bookkeeping(new ThingTalk.Ast.BookkeepingIntent.Answer(value));
                let intent = Intent.fromThingTalk(thingtalk, this._getContext(command, platformData));
                intent.confident = true;
                return this._doHandleCommand(intent, command, []);
            }

            const analyzed = await this.parser.sendUtterance(command, this._context, this._expecting, this._choices);
            if (postprocess)
    ...
    presentExample(utterance, targetCode) {
        return Promise.resolve().then(() => {
            return this._doHandleCommand(Intent.Example(utterance, targetCode), null, [], true);
        }).catch((e) => {
            this.sendReply(this._("Sorry, I had an error processing your command: %s").format(e.message), null);
