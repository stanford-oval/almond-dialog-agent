...
module.exports = class Almond extends events.EventEmitter {
    ...
    async _errorWrap(fn, platformData) {
        try {
            try {
                ...
                if (this._isUnsupportedError(e))
                    await this._doHandleCommand(new Intent.Unsupported(platformData), null, [], true);
                else
                    throw e;
            }
        } catch(e) {
    ...
    handleCommand(command, platformData = {}, postprocess) {
        ...
        return this._errorWrap(async () => {
            if (this._raw && command !== null) {
                const value = new ThingTalk.Ast.Value.String(command);
                const thingtalk = new ThingTalk.Ast.Input.Bookkeeping(null,
                    new ThingTalk.Ast.BookkeepingIntent.Answer(null, value));
    ...
    maybeSendChart(chart, icon) {
        if (this._debug)
            console.log('Almond may send chart with value: ' + JSON.stringify(chart.data));
        return this._delegate.maybeSendChart(chart, icon);
    }
