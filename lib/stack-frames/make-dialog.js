...
async function getCategoryList(dlg, primType) {
    ...
    await dlg.reply(dlg._("Pick one from the following categories or simply type in."));
    if (primType === 'stream')
        await dlg.replySpecial(dlg._("Do it now"), 'empty');
    if (primType === 'action')
        await dlg.replySpecial(dlg._("Just notify me"), 'empty');

    for (let i = 0; i < CATEGORIES.length; i++) {
        const category = CATEGORIES[i];
        const title = titles[i];
        await dlg.replyButton(title, { code: ['bookkeeping', 'category', category], entities: {} });
    }
    if (primType !== 'all')
...
function getDeviceList(dlg, category, page) {
    return dlg.manager.thingpedia.getDeviceList(category, page, PAGE_SIZE).then(async (devices) => {
        await dlg.reply(dlg._("Pick a command from the following devices"));
        ...
        for (let d of devices) {
            ...
            await replyOneDevice(dlg, d.name, d.primary_kind, category);
        }
        if (hasMore)
            await dlg.replySpecial(dlg._("More…"), 'more');
        await dlg.replySpecial(dlg._("Back"), 'back');
    });
}
...
async function getDeviceHelp(dlg, category, name, page, primType, isLocal=true, keywords=[]) {
    ...
    if (examples.length === 0) {
        await dlg.reply(dlg._("Can't find a compatible command from %s, choose another device").format(Helpers.cleanKind(name)));
        await dlg.replySpecial(dlg._("Back"), 'back');
        return;
    }
    ...
    await dlg.reply(dlg._("Pick a command below."));
...
async function expectPredicate(dlg) {
    return dlg.pushStackFrame(new CustomStackFrame(dlg, ValueCategory.Predicate,
        dlg._("I'm looking for a filter."),
        Intent.Back, Intent.Predicate));
}
...
async function fillOnePrimitive(dlg, primType, isLocal=true) {
    ...
    for (;;) {
        if (helpState === 'categoryList')
            await getCategoryList(dlg, primType);
        ...
        let intent = await dlg.pushStackFrame(new CustomStackFrame(dlg, ValueCategory.Command,
            dlg._("I'm looking for a command."),
            Intent.More, Intent.Back, Intent.CommandList, Intent.Empty, Intent.Program));

        if (intent instanceof Intent.More) {
            page++;
            continue;
        }
        ...
        if (intent instanceof Intent.CommandList) {
        ...
        if (intent instanceof Intent.Empty) {
            return primType === 'all' ? [undefined, undefined] : undefined;
        } else {
            assert(intent instanceof Intent.Program);
            if (primType === 'all') {
...
async function makeDialog(dlg, startIntent) {
    let utterances = [];
        ...
        for (;;) {
            ...
            if (index === 3) {
                if (filterCandidates[0].length + filterCandidates[1].length === 0) {
                    await dlg.reply("There is nothing to filter.");
                    continue;
                }
                ...
                await dlg.reply(dlg._("Pick the filter you want to add:"));
                ...
                let filterIntent = await expectPredicate(dlg);
                if (filterIntent instanceof Intent.Back)
                    continue;
    ...
    const intent = new Intent.Program(program, program, startIntent.plaformData);
    intent.confident = true;
    await dlg.pushStackFrame(new ProgramStackFrame(dlg), intent);
}
