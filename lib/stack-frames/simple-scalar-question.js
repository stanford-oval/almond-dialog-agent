...
module.exports = class SimpleScalarQuestionStackFrame extends DialogStackFrame {
    ...
    compatible(command) {
        return command instanceof Intent.Answer || command instanceof Intent.Yes || command instanceof Intent.No;
    }

    async _validate(command) {
        ...
        if (command instanceof Intent.Yes) {
            await this._dlg.reply(this._dlg._("Yes what?"));
            return false;
        }
        if (command instanceof Intent.No) {
    ...
    async dispatch(command) {
        if (!await this._validate(command))
            return;
