...
module.exports = class DialogStackFrame {
    ...
    */
    ...
    /**
      Push a nested stack frame.

      This is a wrapper for Dispatcher.pushStackFrame that also restores the
      context when the nested stack frame is popped.
     */
    async pushStackFrame(stackFrame, intent) {
        const result = await this._dlg.pushStackFrame(stackFrame, intent);
        this.setContext();
        return result;
    }
    ...
    */
    async debug() {
        await this._dlg.reply("I'm in a " + this.constructor.name);
