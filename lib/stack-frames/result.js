...
module.exports = class ResultStackFrame extends DialogStackFrame {
    ...
    async _displayPage() {
        for (let i = this._index; i < Math.min(this._index + PAGE_SIZE, this._results.length); i++) {
            ...
            if (next.isNotification)
                await showNotification(this._dlg, undefined, next.icon, next.outputType, next.outputValue, next.currentChannel, undefined);
            else if (next.isError)
                await showError(this._dlg, undefined, next.icon, next.error, undefined);
        }
    ...
    async _fetchResults() {
        ...
        while (this._results.length < this._index + PAGE_SIZE + 1) {
            ...
            if (next.isQuestion) {
                try {
                    const value = await askAnything(this._dlg, undefined, next.icon, next.type, next.question);
                    resolve(value);
                } catch(e) {
                    reject(e);
                }
