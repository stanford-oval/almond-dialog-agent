...
const AsyncQuestionStackFrame = require('./stack-frames/async-question');
const DeviceDiscoveryStackFrame = require('./stack-frames/discovery');
const DeviceConfigStackFrame = require('./stack-frames/device-config');
...
QueueItem.Notification = class Notification extends QueueItem {
    ...
    async _doDispatch(dlg) {
        ...
        return Helpers.showNotification(dlg, this.appId, this.icon, this.outputType, this.outputValue, previousAppId);
    }
};
...
QueueItem.Error = class Error extends QueueItem {
    ...
    async _doDispatch(dlg) {
        dlg.currentAppId = this.appId;
        return Helpers.showError(dlg, this.appId, this.icon, this.error);
    }
};
...
QueueItem.Question = class Question extends QueueItem {
    ...
    async _doDispatch(dlg) {
        ...
        return dlg.pushStackFrame(new AsyncQuestionStackFrame(dlg, this.appId, this.icon, this.type, this.question), null);
    }
};
...
QueueItem.InteractiveConfigure = class InteractiveConfigure extends QueueItem {
    ...
    async _doDispatch(dlg) {
        dlg.currentAppId = null;
        if (this.kind !== null)
            return dlg.pushStackFrame(new DeviceConfigStackFrame(dlg, this.kind), null);
        else
            return dlg.pushStackFrame(new DeviceDiscoveryStackFrame(dlg), null);
    }
};
